...
class RestApi
{
    ...
    public function call($url, $dataArray = array(), $postType = "GET", $storeCode = 'all', $extraHeaders = [])
    {
        ...
        curl_setopt($handle, CURLOPT_USERAGENT, 'Procucao-Decora-XLMDR3');
        ...
        if ($code == '200') {
            ...
            if ($decodedResponse = json_decode($response)) {
                ...
                if (isset($decodedResponse->parameters)) {
                    ...
                    $exception = "{$code} - Error making request to server: {$decodedResponse->message} - ".json_encode($parameters);
                }
